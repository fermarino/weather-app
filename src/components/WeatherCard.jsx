...
const WeatherCard = ({ location }) => {
  ...
  return (
    <div className="weather-card max-h-[500px] bg-gradient-to-b from-blue-500 to-blue-800 rounded-lg p-8 text-white shadow-lg sm:max-w-[400px] lg:max-w-[400px]">
      <h2 className="text-xl md:text-2xl lg:text-3xl font-semibold mb-2">{name}</h2>
      ...
      <div className="flex-col flex mt-10 gap-11">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-8">
          <WeatherDetails icon={<FaTemperatureHigh />} label="Máxima" value={`${Math.round(main.temp_max)}º`} />
          <WeatherDetails icon={<FaTint />} label="Umidade" value={`${main.humidity}%`} />
          <WeatherDetails icon={<FaWind />} label="Vento" value={`${Math.round(data.wind.speed)} km/h`} />
          <WeatherDetails icon={<FaEye />} label="Visibilidade" value={`${data.visibility / 1000} km`} />
        </div>
      </div>
    </div>
  );
